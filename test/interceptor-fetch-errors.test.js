...
import { readFileSync, existsSync, writeFileSync, mkdirSync, chmodSync, rmSync, mkdtempSync } from 'node:fs';
...
function readEntries() {
  ...
  return readFileSync(mod.LOG_FILE, 'utf-8').split('\n---\n').filter(p => p.trim()).map(p => JSON.parse(p));
}
...
describe('hot-switch active profile 改写路径（baseURL + apiKey）', () => {
  ...
  it('URL 被重写到 profile.baseURL（path 重叠去重）+ auth 替换 + proxyProfile 记账', async () => {
    ...
    // 整块跑完（未在 model 段抛出，因为没设 activeModel）→ proxyProfile 落入日志条目
    const entry = lastCompleted();
    assert.equal(entry.proxyProfile, 'HotProxy');
  });
