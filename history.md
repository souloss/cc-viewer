...
- **Failure policy (injection must never block spawn):** an unloadable user value (unreadable file, invalid JSON, non-object) is dropped with a warning and claude launches with the injected settings only. The warning is localized via a new `cli.settingsMergeFailed` key (all 18 locales) on both surfaces: the embedded terminal via `emitSpawnNotice` (post-spawn, so the boot-time buffer reset can't swallow it; suppressed on internal respawns; the accompanying `console.warn` stays English for greppable server logs) and stderr on the `ccv run` path, which merges for the claude binary only — other tools' own `--settings` args are never touched.
- Coverage: `test/settings-merge.test.js` (33 cases: merge/priority/union semantics, claude-parity argv shapes, byte-identical passthrough for the no-user-flag path incl. the IM deny object, content-failure matrix, hostile non-string argv), end-to-end `spawnClaude` cases in `test/pty-manager-gap.test.js` (exactly one merged `--settings` in the final argv; warning reaches the terminal buffer), and CLI-tier `ccv run` cases in `test/cli-modes.test.js` (merge, unloadable-value warning on stderr, non-claude gate). Six-perspective team review adopted P1+P2: CLI run-path test coverage, localized terminal notice, deny-fallback/non-object-env branch pins, warning-glyph consistency; P3 backlog: `--settings` followed by a literal `--` consumed as value, `console.warn` respawn duplication, raw user value echoed into the terminal buffer intersecting onExit retry-trigger scans (pre-existing pattern).
