...
- feat(wire-v2, S3): **dual-write wired into the interceptor behind `CCV_WIRE_V2` (default OFF)** — the v2 store now records real traffic alongside v1, whose logic is untouched. The five inline completion/error write blocks collapse into one `_writeCompletedEntry()` seam (byte-for-byte the same v1 behavior: inProgress/requestId deletes + `\n---\n` append with the `_commitDeltaState` callback); the rotation sentinel deliberately stays OUT of the seam (baked into file creation). The v2 req-phase ingest runs in the SAME synchronous segment as the v1 `_seq` stamp — no await between them — so v2's journal seq order can never diverge from v1's initiation order (§3.7 guard); it captures the ORIGINAL `body.messages` reference before the delta branch reassigns it to a slice, and replaces the v1 placeholder's role on the v2 side with a body-less journal req line. Completion ingest runs inside the seam while the entry is still intact (rid travels in the handle, immune to the requestId delete). Both call sites carry a belt-and-braces bare catch on top of V2Writer's internal reportSwallowed guards — a sabotaged v2 writer provably cannot reach the v1 write (pinned by test). Lifecycle: `initForWorkspace`/`resetWorkspace` call the new `V2Writer.resetSessions()` (project rebinding → fresh session dirs under the new project; `project` is now a getter option); rotation and resume need no v2 hook (no rotation in v2; fresh process state on resume — decision recorded in WIRE_FORMAT_V2.md §13, including "no `_temp.jsonl` equivalent needed"). Exit handlers drain the v2 queue alongside v1's on SIGINT/SIGTERM/beforeExit. IM workers are pinned v1-only until S6b (`CCV_IM_PLATFORM` excluded from the enable gate); teammates record leader linkage (agentName/teamName/`--parent-session-id`) into session meta. Rollback: unset `CCV_WIRE_V2`. Tests: new `interceptor-v2-dualwrite` (spec-shape assertion over the real fetch hook, blob-ref reuse across requests, sabotage isolation, v1/v2 order agreement) + `interceptor-v2-default-off` (env unset → zero `sessions/` anywhere); all existing interceptor suites pass unmodified (169/169), full `test:cli` 8303 pass / 0 fail.

- feat(wire-v2, S2): **v2 core storage library — pure addition, not wired into the interceptor yet** (spec: `docs/refactor/WIRE_FORMAT_V2.md`). New `server/lib/v2/`: `layout.js` (path contract + component whitelist against injection, synchronous dir creation so AsyncWriteQueue's microtask drain can never hit ENOENT, atomic tmp→fsync→rename for meta.json/blobs, journal self-describing sentinel via create-exclusive write); `identity.js` (**dual-encoding `metadata.user_id` parser** — modern JSON and legacy `_session_<uuid>`-delimited forms both exist in real logs; `ConvResolver` keys sub-conversations by the spawning Agent block's **tool_use.id** with an unclaimed-FIFO per prompt prefix, so parallel same-prompt subagents no longer collide the way v1's overwrite-on-collision prefix registry does — the same-length tie-break prefers claiming a fresh id over stealing a sibling's conversation; fp-ordinal fallback when no spawn is registered); `blob-store.js` (sha256-16 CAS for body.tools/body.system — the fsync durability barrier of the write-order protocol); `journal.js` (two-phase req/done append-only lines, seq allocated at request initiation so completion order can never become semantic order — the §3.7 guard); `conversation-store.js` (exhaustive write-side judgement: unchanged → nothing, same-len tail change → `ctl replace-tail`, prefix growth → `append` slice, everything else → raw `snapshot` for the read-side materializer; /clear via the shared S1 predicate opens a new epoch file, /compact stays in-epoch with a `ctl compact` marker); `v2-writer.js` (orchestrator: blob → conversation → journal write order — the blob half is a hard fsync barrier, the conv half best-effort batch grouping with read-side missing-tail tolerance per spec §14; per-request handles, metadata-less request routing to the active session with a bounded cold-start hold queue, teammate leader-linkage meta, 1GB disk guard, every public method caught + `reportSwallowed` so a v2 failure can never disturb v1). `AsyncWriteQueue` gains an explicit-path `appendTo` variant (same drain machinery; `append` delegates; empty-path no-op semantics preserved). `src/utils/errorReport.js` moved verbatim to `server/lib/error-report.js` (src shell kept) because `src/` is not shipped in the npm package and server-side v2 code must honor the same reportSwallowed convention; `client-safe-imports` allowlist +2. New `test/v2-core.test.js` (28 cases: dual encoding, path injection, epoch/compact/replace-tail/snapshot matrix, parallel same-prompt subagent split — which caught a real continuity-heuristic bug during development — cold-start hold/flush, failure isolation, disk guard, teammate meta, spawn-id keying, write-order spy). Interceptor untouched; v1 behavior zero change.

- refactor(wire-v2, S1): **shared session-boundary + reverse-anchor module** — first code step of the Wire Format v2 log-storage refactor (plan: `docs/refactor/WIRE_FORMAT_V2_PLAN.md`, protocol: `docs/refactor/WIRE_FORMAT_V2.md`). The session-boundary predicates (`isPostClearCheckpoint`/`isCompactContinuation`/`isSessionBoundary`, formerly `src/utils/clearCheckpoint.js`) and the reverse-anchor alignment pair (`messageFingerprint`/`findReverseAnchor`, formerly module-private in `src/utils/sessionMerge.js`) moved VERBATIM into a new CLIENT-SAFE `server/lib/session-boundary.js`, so the upcoming v2 conversation router (S2+) and read-side materializer share ONE source of truth with the client instead of forking the /clear‑vs‑/compact and §3.1/§3.2-window logic. `clearCheckpoint.js` becomes a re-export shell (all existing client imports untouched); `sessionMerge.js` imports the moved pair and keeps re-exporting `messageFingerprint` (test API unchanged); entry-slim's three intentionally-divergent bigDrop mirrors stay where they are per their restore-guard contract. v1 runtime behavior: zero change. New `test/session-boundary.test.js` pins the canonical import path, verifies the shells re-export the same function objects, and adds the first DIRECT `findReverseAnchor` tests (tail anchor, suffix subset, empty-fp rejection, collision-candidate skip, shared fp-cache population); `session-boundary.js` added to the `client-safe-imports` allowlist, whose zero-node-deps scan now guards it.
