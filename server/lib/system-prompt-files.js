 ...
 * the launch directory. Pure function: side effects limited to reading fs/env.
 *
 * @param {string} projectDir 启动目录(绝对路径)
 * @param {string[]} [existingArgs] 已有的 claude 参数(用于「手动优先」判断)
 * @param {Object} [env] 环境变量(默认 process.env)
 * @returns {{ args: string[], loaded: string[] }} args: 待追加参数；loaded: 实际加载的文件名(用于终端提示)
 */
export function buildSystemPromptFileArgs(projectDir, existingArgs = [], env = process.env) {
  const out = { args: [], loaded: [] };
