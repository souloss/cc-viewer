...
Модальное окно **Изменить системный промпт** (Настройки → Экспертные настройки) разделено на вкладки:
...
* **Вкладки моделей**: нажмите **+ Добавить модель**, введите имя, например `opus` или `Gemini3`, и выберите область действия — **Глобально** (`~/.claude/cc-viewer/system_prompt/`, применяется ко всем рабочим областям) или **Рабочая область** (`<project>/system_prompt/`). У каждой вкладки есть собственный переключатель «Добавить/Переопределить» и предпросмотр Markdown.
* Записи хранятся в виде файлов с именами в верхнем регистре: `OPUS_SYSTEM.md` (переопределение) или `OPUS_APPEND_SYSTEM.md` (добавление). Сопоставление нечёткое — ищется нечувствительная к регистру подстрока идентификатора модели, использованного при последнем запуске, поэтому `opus` совпадает с `claude-opus-4-8[1m]` независимо от версии. Совпадение в рабочей области имеет приоритет над глобальным; в пределах одной области побеждает самое длинное имя; совпавшая запись полностью заменяет файлы «По умолчанию» для этого запуска.
